...
const BlogPreview = ({ post, isPreview = false }: BlogContentProps) => (
  <main className={`${isPreview ? styles.main_preview : styles.main}`}>
    <header className={`${isPreview ? styles.header_preview : styles.header}`}>
      <div
        className={`${isPreview ? styles.header_div_preview : styles.header_div}`}
      >
    ...
    {post.sections.map((section, index) => (
      <section
        ...
        key={index}
      >
        <h2 className={isPreview ? styles.h2_preview : ""}>
          {section.heading}
        </h2>
        {section.content &&
          section.content.map((contentBlock, idx) => (
            <div
              className={`${isPreview ? styles.div_preview : styles.div}`}
              key={idx}
            >
              {contentBlock.points && (
                <ul className={`${isPreview ? styles.ul_preview : styles.ul}`}>
                  {contentBlock.points.map((point) => (
                    <li
                      className={`${isPreview ? styles.li_preview : styles.li}`}
                      key={point.id}
                    >
                      <p className={isPreview ? styles.p_preview : ""}>
                        {point.value}
      ...
      >
        ...
        {post.faq.map((faq, i) => (
          <div
            ...
            key={i}
          >
            <strong className={isPreview ? styles.strong_preview : ""}>
              {faq.question}
            </strong>
